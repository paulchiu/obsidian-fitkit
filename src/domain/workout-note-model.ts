 ...
 * and any other body line (prose, ordinary headings, blockquotes,
 ...
 * inline field.
 */
function exerciseNameFromModeledBulletLine(line: string): string | null {
  if (/^\s/.test(line)) {
    return null
  }
  ...
  const exerciseField = collectInlineFields(bullet[1]).get('exercise')
  return exerciseField === undefined ? null : unwrapWikiLink(exerciseField)
}

function isModeledBulletLine(line: string): boolean {
  return exerciseNameFromModeledBulletLine(line) !== null
}

function findExerciseHeadingIndexes(lines: Array<string | null>): Set<number> {
  const indexes = new Set<number>()

  for (let headingIndex = 0; headingIndex < lines.length; headingIndex += 1) {
    const heading = lines[headingIndex]
    if (typeof heading !== 'string' || !H2.test(heading.trim())) {
      continue
    }
...
/**
 * Include separator blanks beside preserved content unless the serializer
 * owns that spacing. It emits the leading blank after frontmatter and exercise
 * headings, but not the trailing blank before a modeled row.
 */
function findPreservedBlockRange(
  bodyLines: string[],
  contentStart: number,
  contentEnd: number,
  exerciseHeadingIndexes: Set<number>,
): { start: number; end: number } {
  let start = contentStart
  while (start > 0 && (bodyLines[start - 1] ?? '').trim() === '') {
    start--
  }
  const previousNonBlank = start - 1
  if (previousNonBlank < 0 || exerciseHeadingIndexes.has(previousNonBlank)) {
    start = contentStart
  }
  ...
  if (end >= bodyLines.length || !isModeledBulletLine(bodyLines[end] ?? '')) {
    end = contentEnd
  }
 ...
 * checkboxes), nested bullets, ordinary headings, and other prose. An H2 is
 * an exercise heading only when its section contains a modeled row. Adjacent
 * non-modeled lines are grouped into a single block so interior blank lines
 * survive. Separator blanks after a modeled row are included in the following
 * block, while spacing between modeled elements remains generated by the
 * serializer. Blocks share the same raw (original-line-index) coordinate
 * system as the fenced-block blocks above; `parseWorkoutNote` converts all of
 * them into section-anchored `PreserveBlock`s in a single pass afterward.
 */
function extractPreserveBlocks(bodyLines: string[]): ExtractResult {
  ...
  const exerciseHeadingIndexes = findExerciseHeadingIndexes(cleaned)
  const blocks: RawBlock[] = fencedBlocks.map((block) => {
    const contentEnd = block.index + block.text.split('\n').length
    const { start, end } = findPreservedBlockRange(
      bodyLines,
      block.index,
      contentEnd,
      exerciseHeadingIndexes,
    )
    for (let k = start; k < end; k++) {
      cleaned[k] = null
    }
    return {
      index: start,
      text: bodyLines.slice(start, end).join('\n'),
    }
  })
  ...
  const isBoundary = (idx: number): boolean => {
    ...
    return exerciseHeadingIndexes.has(idx) || isModeledBulletLine(line)
  }
  ...
  while (i < cleaned.length) {
    ...
    let realEnd = end
    while (realEnd > contentStart && isBlank(realEnd - 1)) {
      realEnd--
    }

    const { start, end: preservedEnd } = findPreservedBlockRange(
      bodyLines,
      contentStart,
      realEnd,
      exerciseHeadingIndexes,
    ...
    blocks.push({ index: start, text: bodyLines.slice(start, preservedEnd).join('\n') })
    for (let k = start; k < preservedEnd; k++) {
      cleaned[k] = null
    }
    i = end
  }
