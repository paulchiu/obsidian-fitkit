...
const DURATION_RECENT_SESSIONS_BLOCK = [
  ...
  'where contains(item.text, "[exercise:: [[Mystery]]]") and item.duration',
  'sort file.name desc',
  'limit 12',
  '```',
].join('\n')
...
`
    ...
    const result = setExerciseNoteKind(source, 'strength', kindOptions)
...
`
  ...
  it('leaves a hand-edited Recent sessions query alone and reports it', () => {
    const customBlock = [
      '```dataview',
      'TABLE WITHOUT ID',
      '  file.link AS Workout,',
  ...
  it('leaves the chart block, the Notes query and prose untouched', () => {
    const notesBlock = buildNotesBlock('Mystery', 'Fitness')
    const source = `---
type: exercise
kind: strength
metric: e1rm
...
${buildRecentSessionsBlock('Mystery', 'strength', 'Fitness')}
...
`
    ...
    expect(extractDataviewBlockAfter(result.markdown, 'Notes')).toBe(notesBlock)
    expect(extractDataviewBlockAfter(result.markdown, 'Recent sessions')).toBe(
      DURATION_RECENT_SESSIONS_BLOCK,
    )
  })
